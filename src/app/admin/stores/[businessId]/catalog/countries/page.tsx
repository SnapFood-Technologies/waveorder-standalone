    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <Globe className="w-7 h-7 text-teal-600 shrink-0" />
      ...
      {loading && (
        <div className="flex justify-center py-16">
          <Loader2 className="w-8 h-8 animate-spin text-teal-600" aria-label="Loading" />
        </div>
      )}

      {error && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-5 py-4 lg:px-6 text-sm text-red-800">{error}</div>
      )}
        ...
        <div className="space-y-6">
          <section className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            {visibleRows.length === 0 ? (
              <p className="px-5 lg:px-6 py-10 text-sm text-gray-500 text-center">No visible-country assignments yet.</p>
            ) : (
              <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-5 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Country</th>
                    <th className="px-5 lg:px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Products</th>
                    <th className="px-5 lg:px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase w-40">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {visibleRows.map((r) => (
                    <tr key={r.code} className="hover:bg-gray-50">
                      <td className="px-5 lg:px-6 py-3 text-sm text-gray-900">{labelForCode(r.code)}</td>
                      <td className="px-5 lg:px-6 py-3 text-right text-gray-700">{r.productCount}</td>
          ...
          <section className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-5 lg:px-6 py-4 bg-amber-50 border-b border-amber-100">
            ...
            {excludedRows.length === 0 ? (
              <p className="px-5 lg:px-6 py-10 text-sm text-gray-500 text-center">No excluded-country assignments yet.</p>
            ) : (
              <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
