...
/**
 * Build actual public URL from request headers (avoids logging localhost in proxy setups)
 */
export function getActualRequestUrl(request: Request, fallbackPath = '/'): string {
  try {
    const headers = (request as any).headers || new Headers()
    const host = headers.get('host') || headers.get('x-forwarded-host')
    const protocol = headers.get('x-forwarded-proto') || (process.env.NODE_ENV === 'production' ? 'https' : 'http')
    const parsed = request.url ? new URL(request.url) : null
    const path = parsed ? `${parsed.pathname}${parsed.search}` : fallbackPath
    return host ? `${protocol}://${host}${path}` : (request.url || fallbackPath)
  } catch {
    return request.url || fallbackPath
  }
}
