/**
 * Shared external product sync (ByBest-style). Used by SuperAdmin UI and cron triggers.
 * Query string (same as SuperAdmin): per_page, current_page, sync_all_pages=true|false
 */
import { NextRequest, NextResponse } from 'next/server'
import { prisma } from '@/lib/prisma'
import { sendSuperAdminExternalSyncNotification } from '@/lib/superadmin-email-notification'

export async function runExternalSyncOperation(
  request: NextRequest,
  businessId: string,
  syncId: string
  ...
  /** Set after sync row is loaded; used for SuperAdmin emails if the run throws */
  let syncMetaForEmail: {
    displayName: string
  ...
  try {
    ...
    syncMetaForEmail = {
      displayName:
        extractName(sync.name).en ||
        extractName(sync.name).sq ||
        'External sync',
      externalSystemName: sync.externalSystemName ?? null
    }
    ...
    void sendSuperAdminExternalSyncNotification(businessId, {
      syncName: syncMetaForEmail?.displayName ?? 'External sync',
      externalSystemName: syncMetaForEmail?.externalSystemName ?? undefined,
      status: syncStatus,
      processedCount,
      skippedCount,
      errorCount: errors.length,
      ...
      logId: syncLog?.id ?? undefined
    }).catch((err) =>
      console.error('[Sync] SuperAdmin external sync email failed:', err)
    )
    ...
    if (syncMetaForEmail) {
      const durationMs = syncStartTime
        ? Date.now() - syncStartTime.getTime()
        : undefined
      void sendSuperAdminExternalSyncNotification(businessId, {
        syncName: syncMetaForEmail.displayName,
        externalSystemName: syncMetaForEmail.externalSystemName ?? undefined,
        status: 'failed',
        durationMs,
        lastError:
