...
export default function ServicesStoreFront({ storeData }: { storeData: StoreData }) {
  ...
  const allowRequestByEmail = storeData.serviceAllowRequestByEmail ?? false
  const allowRequestByWhatsApp = storeData.serviceAllowRequestByWhatsApp ?? false
  const [serviceRequestForm, setServiceRequestForm] = useState({
    requesterType: 'PERSON' as 'PERSON' | 'COMPANY',
    contactName: '',
    companyName: '',
    email: '',
    phone: '',
    ...
    message: '',
    preferredContact: (allowRequestByEmail ? 'EMAIL' : 'WHATSAPP') as 'EMAIL' | 'WHATSAPP'
  })
  ...
  const handleServiceRequestSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const allowEmail = storeData.serviceAllowRequestByEmail ?? false
    const allowWhatsApp = storeData.serviceAllowRequestByWhatsApp ?? false
    if (!allowEmail && !allowWhatsApp) return
    const preferred = serviceRequestForm.preferredContact === 'WHATSAPP' ? 'WHATSAPP' : 'EMAIL'
    if (preferred === 'EMAIL' && !allowEmail) return
    if (preferred === 'WHATSAPP' && !allowWhatsApp) return
    setServiceRequestSubmitting(true)
    try {
      const res = await fetch(`/api/storefront/${storeData.slug}/service-request`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          requesterType: serviceRequestForm.requesterType,
          contactName: serviceRequestForm.contactName.trim(),
          companyName: serviceRequestForm.requesterType === 'COMPANY' ? serviceRequestForm.companyName.trim() || undefined : undefined,
          email: serviceRequestForm.email.trim(),
          phone: serviceRequestForm.phone.trim() || undefined,
          serviceIds: serviceRequestForm.serviceIds.length ? serviceRequestForm.serviceIds : undefined,
          message: serviceRequestForm.message.trim() || undefined,
      ...
      if (res.ok) {
        ...
        setServiceRequestForm({
          ...
          serviceIds: [],
          message: '',
          preferredContact: allowEmail ? 'EMAIL' : 'WHATSAPP'
        })
      } else {
        const data = await res.json().catch(() => ({}))
        showError(data.error || translations.somethingWentWrong)
      }
    } catch (err) {
      ...
      {(storeData.serviceAllowRequestByEmail || storeData.serviceAllowRequestByWhatsApp) && (
        <section className="max-w-[75rem] mx-auto px-5 py-8 mt-8 border-t border-gray-200">
          ...
          {serviceRequestSuccess ? (
            <div className="rounded-lg p-4 bg-green-50 text-green-800 text-sm">
              {translations.requestReceived}
            </div>
          ) : (
            <form onSubmit={handleServiceRequestSubmit} className="space-y-4 max-w-xl">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{translations.requestFormIAm}</label>
                <div className="flex gap-4">
                  <label className="inline-flex items-center gap-2">
                    <input
                      type="radio"
                      name="requesterType"
                      checked={serviceRequestForm.requesterType === 'PERSON'}
                      onChange={() => setServiceRequestForm(f => ({ ...f, requesterType: 'PERSON' }))}
                      className="rounded border-gray-300"
                    />
                    <span className="text-sm">{translations.requestFormPerson}</span>
                  </label>
                  <label className="inline-flex items-center gap-2">
                    <input
                      type="radio"
                      name="requesterType"
                      checked={serviceRequestForm.requesterType === 'COMPANY'}
                      onChange={() => setServiceRequestForm(f => ({ ...f, requesterType: 'COMPANY' }))}
                      className="rounded border-gray-300"
                    />
                    <span className="text-sm">{translations.requestFormCompany}</span>
                  </label>
                </div>
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={e => setServiceRequestForm(f => ({ ...f, contactName: e.target.value }))}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                  placeholder={translations.yourFullName}
                />
              </div>
              {serviceRequestForm.requesterType === 'COMPANY' && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{translations.companyName}</label>
                  <input
                    type="text"
                    value={serviceRequestForm.companyName}
                    onChange={e => setServiceRequestForm(f => ({ ...f, companyName: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setServiceRequestForm(f => ({ ...f, email: e.target.value }))}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                  placeholder={translations.emailPlaceholder}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{translations.phone}</label>
                <input
                  type="tel"
                  value={serviceRequestForm.phone}
                  onChange={e => setServiceRequestForm(f => ({ ...f, phone: e.target.value }))}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                  placeholder="+1 234 567 8900"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{translations.notes}</label>
                <textarea
                  rows={3}
                  value={serviceRequestForm.message}
                  onChange={e => setServiceRequestForm(f => ({ ...f, message: e.target.value }))}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                  placeholder={translations.anySpecialRequests}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{translations.requestFormPreferredContact}</label>
                <div className="flex gap-4">
                  {storeData.serviceAllowRequestByEmail && (
                    <label className="inline-flex items-center gap-2">
                      <input
                        ...
                        checked={serviceRequestForm.preferredContact === 'EMAIL'}
                        onChange={() => setServiceRequestForm(f => ({ ...f, preferredContact: 'EMAIL' }))}
                        className="rounded border-gray-300"
                      />
                  ...
                  {storeData.serviceAllowRequestByWhatsApp && (
                    <label className="inline-flex items-center gap-2">
                      <input
                        type="radio"
                        name="preferredContact"
                        checked={serviceRequestForm.preferredContact === 'WHATSAPP'}
                        onChange={() => setServiceRequestForm(f => ({ ...f, preferredContact: 'WHATSAPP' }))}
                        className="rounded border-gray-300"
                      />
              ...
              <button
                ...
                style={{ backgroundColor: primaryColor }}
              >
                {serviceRequestSubmitting ? translations.submitting : translations.submitRequest}
              </button>
            </form>
          )}
        </section>
