// src/components/admin/analytics/ServiceAnalytics.tsx
'use client'
...
interface ServiceData {
  ...
  views: number
  addToCarts: number
  appointmentsBooked: number      // All appointments (customer intent)
  appointmentsCompleted: number   // Completed appointments only
  revenue: number
  quantityBooked: number
  quantityCompleted: number
  viewToCartRate: number
  cartToAppointmentRate: number
...
interface AnalyticsData {
  summary: {
    ...
    totalAppointmentsBooked: number     // All appointments (customer intent)
    totalAppointmentsCompleted: number  // Completed appointments only
    totalRevenue: number
    overallViewToCartRate: number
    overallCartToAppointmentRate: number
  ...
  bestSellers: ServiceData[]
  mostViewed: ServiceData[]
  opportunityServices: ServiceData[]
  lowPerforming: ServiceData[]
  period: string
  dateRange: {
    start: string
    end: string
...
export default function ServiceAnalytics({ businessId }: ServiceAnalyticsProps) {
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      const response = await fetch(
        `/api/admin/stores/${businessId}/analytics/services?period=${selectedPeriod}&limit=10`
      )
  ...
  const getServicesList = () => {
    ...
    switch (activeTab) {
      ...
      case 'opportunity':
        return data.opportunityServices
      case 'low-performing':
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Track service performance and conversion rates
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <p className="text-sm text-gray-600">Total Service Views</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center p-4 bg-emerald-50 rounded-lg">
              ...
              <p className="text-xs text-emerald-500 mt-1">
                Sessions that completed booking
              </p>
            </div>
          </div>
                ...
                {services.map((service, index) => (
                  <tr key={service.serviceId} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="text-sm text-gray-900">{formatNumber(service.views)}</span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className="text-sm text-gray-900">{formatNumber(service.addToCarts)}</span>
                      {service.views > 0 && (
                        <span className="text-xs text-gray-500 block">
                          {service.viewToCartRate}%
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className="text-sm font-medium text-teal-700">{service.appointmentsBooked}</span>
                      <span className="text-xs text-gray-500 block">
                        {service.quantityBooked} bookings
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className="text-sm font-medium text-green-700">{service.appointmentsCompleted}</span>
                      <span className="text-xs text-gray-500 block">
                        {service.quantityCompleted} bookings
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        service.conversionRate >= 5
                          ? 'bg-green-100 text-green-800'
                          : service.conversionRate >= 2
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link
                        href={addParams(`/admin/stores/${businessId}/services/${service.serviceId}`)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <ul className="text-xs space-y-1 text-blue-700">
                  <li><strong>Views:</strong> When a customer opens a service modal</li>
                  <li><strong>Add to Cart:</strong> When a customer adds a service to their cart</li>
                </ul>
              </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex gap-3">
                      ...
                      <div>
                        <p className="font-medium text-gray-800">Total Service Views</p>
                        <p className="text-gray-600">Number of times customers opened service details. Each view indicates customer interest.</p>
                      </div>
                    </div>
                    <div className="flex gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">Number of times services were added to cart. Shows booking intent.</p>
                      </div>
                    </div>
                    <div className="flex gap-3">
                    ...
                    <div className="flex gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">Total money earned from completed appointments only. Does not include pending appointments.</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-purple-50 rounded-lg p-4">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <p className="text-xs text-purple-600 mt-1">Higher = Services are appealing to customers</p>
                    </div>
                    <div>
                ...
                <div className="bg-teal-50 rounded-lg p-4">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <p className="text-gray-600">How many times this specific service was viewed</p>
                    </div>
                    <div>
                      <p className="font-medium text-gray-800">Add to Cart (with %)</p>
                      <p className="text-gray-600">Number of cart additions. The percentage shows view-to-cart rate for this service.</p>
                      <p className="text-xs text-teal-600 mt-1">Example: "2 (100%)" = 2 add-to-carts from 2 views</p>
                      <p className="text-xs text-orange-600">Note: Can exceed 100% if service is added multiple times without viewing</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">Overall conversion rate for this service. Formula: (Appointments Booked ÷ Views) × 100</p>
                ...
                <div className="bg-orange-50 rounded-lg p-4">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">Number of sessions with items left in cart without completing booking.</p>
                    </div>
                    <div>
                      <p className="font-medium text-gray-800">Converted Carts</p>
                      <p className="text-gray-600">Number of sessions that successfully completed a booking.</p>
                    </div>
                    <p className="text-xs text-orange-600 mt-2">
                      Tip: High abandonment rate may indicate issues with checkout process, pricing, or payment options.
                    </p>
                  </div>
                </div>
              </div>
